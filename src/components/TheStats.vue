...
<template>
  <ul class="divide-y">
    <li v-for="time, activity in goals" class="p-4 flex justify-between items-center">
      <span>{{ activity }}</span>
      <div class="flex">
        {{ currentTime(activity) }} / {{ time }}
        <div class="ml-4 px-2 rounded"
          :class="time - currentTime(activity) < 0 ? 'text-red-600 bg-red-100' : 'text-green-600 bg-green-100'">
